...
a_1 = [ones(m, 1), X];
z2 = Theta1 * a_1';
a2 = sigmoid(z2); % a2 = |a2(1), ... a2(i), .... a2(m)| 
a_2 = [ones(1, m); a2];
z3 = Theta2 * a_2;
a3 = sigmoid(z3);
...
J = -1/m * sum((labels .* log(a3)' + (1 - labels) .* log(1 - a3)') * ones(size(labels, 2), 1))...
    + lambda/(2*m) * (sum(Temp1(:).^2) + sum(Temp2(:).^2));
...
for i = 1:m
    ...
    delta3 = a3(:, i) - (labels(i, :))';
    delta2 = Theta2(:, 2:end)' * delta3.*sigmoidGradient(z2(:, i));
    
    Theta2_grad = Theta2_grad + 1/m * delta3*a_2(:, i)';
    Theta1_grad = Theta1_grad + 1/m * delta2*a_1(i, :);
